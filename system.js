...
// Fungsi helper untuk pemutaran audio dengan error handling terpusat
...
// Pemutar utama
    ...
    if (playIcon1 && pauseIcon1 && !artistPlayPending) {
        if (currentArtistSong && combinedPlaylist[currentArtistSong] && combinedPlaylist[currentArtistSong].artist === currentArtist && isPlaying) {
...
// Fungsi helper untuk memperbarui ikon ke status play
function updateIconsToPlay() {
...
function setupPlayPauseToggle(playIcon, pauseIcon, isArtist = false) {
    ...
    pauseIcon.addEventListener('click', () => {
        ...
        if (isArtist) {
            if (!currentArtist) {
                // tidak ada artis saat ini, abaikan klik
                updatePlayPauseIcons();
                return;
            }
            if (songEnded) {
                // Lagu dari artis baru setelah lagu sebelumnya berakhir
                songEnded = false;
                updateIconsToPlay();
                ...
                // Cek apakah lagu dari artis saat ini sedang diputar
                if (!audioPlayer.paused) {
                    // Saat ini sedang diputar, pause lagu
                    audioPlayer.pause();
                    updatePlayPauseIcons();
                } else {
                    // Lagu dari artis saat ini dalam keadaan pause, lanjutkan pemutaran
                ...
                // Putar lagu acak dari artis saat ini
                updateIconsToPlay();
                const randomSongTitle = getRandomArtistSong(currentArtist);
                if (randomSongTitle) {
                    playSong(randomSongTitle, true);
            ...
            // Mainkan atau lanjutkan pemutaran lagu utama
            updateIconsToPlay();
            safePlayAudio();
        }
    });
...
function updateMiniPlayer() {
    ...
    if (currentArtistSong && combinedPlaylist[currentArtistSong]) {
        ...
        mobileContent.style.paddingBottom = '45px';
...
audioPlayer.addEventListener('play', () => { isPlaying = true; updatePlayPauseIcons(); });
