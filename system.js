...
// Membuat objek lagu yang dipetakan berdasarkan judul lagu untuk kemudahan akses
...
// ===================== STATE APLIKASI =====================
// State untuk melacak lagu yang sudah dimainkan dan history lagu
const appState = {
    playedSongs: [],
    songHistory: [],
...
function playSong(songKey) {
    ...
    appState.songEnded = false;

    // Update current artist and song
    appState.currentArtistSong = songKey;
    appState.currentArtist = songData.artist;

    // Tambahkan lagu ke history untuk back navigation
    appState.songHistory.push(songKey);
    if (appState.songHistory.length > 10) {
        appState.songHistory.shift(); // Batasi maksimal 10 lagu di history
    }

    // Tambahkan lagu ke daftar lagu yang sudah dimainkan
    if (!appState.playedSongs.includes(songKey)) {
        appState.playedSongs.push(songKey);
        // Batasi maksimal 5 lagu terakhir untuk menghindari pengulangan
        if (appState.playedSongs.length > 5) {
            appState.playedSongs.shift();
        }
    }
    ...
    gambarMusik.innerHTML = '<img src="' + songData.image + '" alt="Album" class="player-img" style="-webkit-user-drag: none; -moz-user-drag: none; -ms-user-drag: none;">';
    ...
    // Mini player akan diupdate melalui event listener audioPlayer
}
...
function getRandomSong() {
    ...
    if (appState.playedSongs.length >= playlistKeys.length) {
        appState.playedSongs = [];
    }

    let availableSongs = playlistKeys.filter(song => !appState.playedSongs.includes(song));
...
function backSong() {
    if (appState.songHistory.length > 1) {
        appState.songHistory.pop(); // Hapus current song dari history
        const previousSong = appState.songHistory[appState.songHistory.length - 1]; // Ambil lagu sebelumnya
        if (previousSong) {
...
audioPlayer.addEventListener('ended', function() {
    ...
    // jika loop song aktif
        ...
        audioPlayer.currentTime = 0;
        audioPlayer.play();
    // Jika tidak ada loop atau random aktif, update icons to pause state
    } else {
        updatePlayPauseIcons();
    }
});
...
function generateSongList(artistName) {
    ...
    return artistSongs.map(song => `
        <div class="song-item" data-song="${song.title}">
            <div class="song-kiri">
                <img src="${song.image}" alt="${song.title}" style="-webkit-user-drag: none; -moz-user-drag: none; -ms-user-drag: none;">
                <p>${song.title}</p>
            </div>
...
function updateSongArtis(artistName) {
    ...
    if (playIcon1 && pauseIcon1) {
        if (appState.currentArtistSong && combinedPlaylist[appState.currentArtistSong] && combinedPlaylist[appState.currentArtistSong].artist === appState.currentArtist && !audioPlayer.paused) {
...
function setupPlayPauseToggle(playIcon, pauseIcon, isArtist = false) {
    ...
    pauseIcon.addEventListener('click', () => {
        if (isArtist) {
            // For artist player, toggle play/pause or play random song
            if (appState.currentArtistSong && combinedPlaylist[appState.currentArtistSong].artist === appState.currentArtist) {
                if (audioPlayer.paused) {
                    audioPlayer.play();
                } else {
                    audioPlayer.pause();
                ...
                // Play random song from current artist
                const artistSongs = songsByArtist[appState.currentArtist];
                if (artistSongs && artistSongs.length > 0) {
                    const randomIndex = Math.floor(Math.random() * artistSongs.length);
                    playSong(artistSongs[randomIndex].title);
...
// ===================== EVENT LISTENERS =====================
// Setup play/pause toggle for main player
document.querySelector('.Song-Artis').addEventListener('click', (e) => {
...
document.querySelector('#closePlayer1 .navbar-container1 svg').addEventListener('click', function() {
    ...
    // Show mini player if song is active
    if (appState.currentArtistSong) {
        updateMiniPlayer();
    }
});
...
document.querySelectorAll('.album-list2 a').forEach(function(album) {
    album.addEventListener('click', function(e) {
        ...
        if (Random && musik) {
            Random.innerHTML = '<img src="' + musik + '" alt="Album" class="player-img2" style="-webkit-user-drag: none; -moz-user-drag: none; -ms-user-drag: none;">';
        }
...
function updateMiniPlayer() {
    ...
    if (appState.currentArtistSong && combinedPlaylist[appState.currentArtistSong]) {
        const songData = combinedPlaylist[appState.currentArtistSong];
        miniGambarMusik.innerHTML = '<img src="' + songData.image + '" alt="Mini Album" style="width: 100%; height: 100%; border-radius: 4px; object-fit: cover; -webkit-user-drag: none; -moz-user-drag: none; -ms-user-drag: none;">';
        miniLagu.textContent = appState.currentArtistSong;
...
// Update play/pause icons on audio events
