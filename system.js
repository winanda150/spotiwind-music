...
// Variabel untuk menyimpan elemen yang di-cache
...
// Mengecek apakah perangkat saat ini adalah mobile
function Mobile() {
...
function setupPlayPauseToggle(playIcon, pauseIcon, isArtist = false) {
    ...
    pauseIcon.addEventListener('click', debounce(() => {
        if (isArtist) {
            if (songEnded) {
                // Update ikon segera dengan requestAnimationFrame untuk responsivitas yang lebih baik
                ...
                if (artistSongs && artistSongs.length > 0) {
                    ...
                    playSong(randomSong.title, true);
                }
                ...
                if (!audioPlayer.paused) {
                    ...
                    // Asumsikan pemutaran akan berhasil, update ikon segera dengan requestAnimationFrame
                    requestAnimationFrame(() => {
                        if (playIcon && pauseIcon) {
                            playIcon.style.display = 'block';
                            pauseIcon.style.display = 'none';
                ...
                // Update icons immediately with requestAnimationFrame
                requestAnimationFrame(() => {
                    if (playIcon && pauseIcon) {
                        playIcon.style.display = 'block';
                        pauseIcon.style.display = 'none';
...
albumList1.forEach(function(album) {
    album.addEventListener('click', function(e) {
        e.preventDefault();
        // Tutup pemutar artis saat membuka pemutar album
        document.getElementById('playerOverlay2').classList.remove('active');
...
function updateMiniPlayer() {
    ...
    // Sembunyikan mini player jika player utama aktif untuk menghindari duplikasi kontrol
    if (playerOverlay.classList.contains('active')) {
        miniPlayer.style.display = 'none';
        return;
    }

    // Tampilkan mini player jika ada lagu yang sedang diputar
    if (currentArtistSong && combinedPlaylist[currentArtistSong]) {
        const songData = combinedPlaylist[currentArtistSong];
        // Update gambar album, judul lagu, dan nama artis
